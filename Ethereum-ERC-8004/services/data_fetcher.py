"""Data fetching service for external APIs (Etherscan, CoinGecko, DEX)."""
...
class DataFetcher:
    ...
    def __init__(self):
        ...
        # Blockscout API for Ethereum (free, no API key needed)
        self.blockscout_base_url = "https://eth.blockscout.com/api/v2"
    ...
    async def _make_etherscan_request(self, params: dict) -> dict:
        ...
        params["apikey"] = self.settings.etherscan_api_key or ""
    ...
    async def get_holder_count(self, token_address: str) -> int:
        ...
        # Fallback: Etherscan transfer estimation
        try:
            params = {
                "module": "account",
                "action": "tokentx",
    ...
    async def get_token_price_coingecko(self, token_address: str) -> Optional[float]:
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                url = f"{self.coingecko_base_url}/simple/token_price/ethereum"
                params = {
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    if pairs:
                        ...
                        for pair in pairs:
                            # Filter for Ethereum mainnet pairs
                        ...
                        result = {
                            ...
                            "price_usd": price_usd,
                            "pairs_count": len([p for p in pairs if p.get("chainId") == "ethereum"]),
                            "age_days": age_days
                        }
                        _cache[cache_key] = result
