...
class TokenAnalyzer:
    ...
    def _generate_analysis(
        ...
        risk_descriptions = {
            RiskLevel.SCAM: "Ce token presente de nombreux signes de risque eleve et pourrait etre une arnaque.",
            RiskLevel.HIGH_RISK: "Ce token presente plusieurs indicateurs de risque importants.",
            RiskLevel.MODERATE: "Ce token presente quelques risques mais semble relativement etabli.",
            RiskLevel.QUALITY: "Ce token presente des indicateurs de qualite solides."
        }

        analysis = f"{symbol} a obtenu un score de {score}/100. {risk_descriptions[risk_level]}"

        if metrics.holders > 0:
            analysis += f" Le token compte {metrics.holders:,} holders."

        if metrics.liquidity_usd > 0:
            analysis += f" Liquidite: ${metrics.liquidity_usd:,.0f}."

        if red_flags:
            high_severity = [f.description for f in red_flags if f.severity == "high"]
            if high_severity:
                analysis += f" Alertes critiques: {'; '.join(high_severity)}"
